...
public class FluidTankBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    protected ItemActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory namedScreenHandlerFactory = (FluidTankBlockEntity) world.getBlockEntity(pos);
    ...
    public @Nullable <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return validateTicker(type, ModBlockEntities.FLUID_TANK, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1, pos, state1));
    }
}
