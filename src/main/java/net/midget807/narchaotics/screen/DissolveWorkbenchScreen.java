...
public class DissolveWorkbenchScreen extends HandledScreen<DissolveScreenHandler> {
    ...
    public DissolveWorkbenchScreen(DissolveScreenHandler handler, PlayerInventory inventory, Text title) {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        context.drawTexture(DISSOLVE_TEXTURE, x, y, 0, BACKGROUND_X_OFFSET, 0, BACKGROUND_WIDTH, BACKGROUND_HEIGHT, 256, 256);
    ...
    private void renderProgressArrow(DrawContext context, int x, int y) {
        if (this.handler.isCooking()) {
            context.drawTexture(DISSOLVE_ARROW, x + 91, y + 63, 0, 0, 0, this.handler.getScaledArrowProgress(), 16, 38, 16);
        }
    }

    private void renderFlame(DrawContext context, int x, int y) {
        context.drawTexture(this.handler.isSoulBurner() ? FLAME_SOUL : FLAME, x + 102, y + 84, 0, 0, 0, 14, 14, 14, 14);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        drawFluid(context, handler.blockEntity.productFluidStorage1.variant, 137, 61, 22, ModScreenUtil.getHeightForVolume(handler.blockEntity.productFluidStorage1, 22), 22);
